...
public class Parser {
    ...
    public Parser(String SourceFileNm, SymbolTable st) throws Exception {
        this.scan = new Scanner(SourceFileNm, debugger);
        this.debugger = new Debug();
        this.st = st;
        this.validDataTypes = new HashMap<Integer, Integer>();
        this.validDataTypes.put(2, 3); // Integer -> Float
        this.validDataTypes.put(3, 2); // Float -> Integer
        this.validDataTypes.put(4, 5); // bool -> String
        this.validDataTypes.put(5, 4); // String -> bool
        this.validDataTypes.put(5, 2); // String -> bool
    }
    ...
    public void statements() throws Exception, ParserException {
        ...
        while (true) {
            ...
            if (scan.currentToken.tokenStr.equals("print")) {
                ...
                this.debugger.setDebugState(this.scan);
            ...
            if (scan.nextToken.tokenStr.equals(";")) {
                if (ltype != rToken.subClassif) {
                    if ((ltype == Token.INTEGER && rToken.subClassif == Token.FLOAT) || (ltype == Token.FLOAT && rToken.subClassif == Token.INTEGER)) {
                        ...
                        msg += ltype;
                        msg += ", ";
                        msg += rToken.subClassif;
                        throw new ParserException(
                            scan.currentToken.iSourceLineNr, msg,
    ...
    public void ifStmt() throws Exception {
        boolean done = false;
        while (!done){
            boolean condition = evalCond().value.equals("true");
            if (condition){
                this.statements();
                done = true;
                this.skipIf(true);
                ...
                this.skipIf(false);
                if (this.scan.currentToken.tokenStr.equals("else") && this.scan.nextToken.tokenStr.equals(":")){
                    this.statements();
                    done = true;
                    this.skipIf(true);
                } else if (this.scan.currentToken.tokenStr.equals("else")) {
                    this.scan.getNext();
                }
            }
        }
    ...
    public void skipIf(boolean all) throws Exception{
        int count = 1;
        while (true){
            ...
            } else if (this.scan.currentToken.tokenStr.equals("if")) {
                count ++;
            }

            if (this.scan.getNext().isEmpty())
                throw new Exception();
    ...
    public void whileStmt() throws Exception  {
        ...
        while (whileCount != 0) {
            if (scan.getNext().isEmpty()) {
                throw new ParserException(
                    scan.currentToken.iSourceLineNr,
                    "No terminating 'endwhile' token for while loop",
                    scan.sourceFileNm, "");
            }
        ...
        endBuffer = old.buffer.length() - scan.buffer.length();
        endLine = scan.line;
        scan.getNext();
        endToken = scan.nextToken;


        this.scan = old.saveState();
        this.scan.buffer = this.scan.buffer.substring(0, endBuffer);

        while (evalCond().value.equals("true")) {
            statements();
            this.scan = old.saveState();
            scan.buffer = scan.buffer.substring(0, endBuffer);
        }
        this.scan = old.saveState();
        scan.buffer = scan.buffer.substring(endBuffer);
        scan.line = endLine;
        scan.nextToken = endToken;
    }
    ...
    public ResultValue evalCond() throws Exception {
        ...
        */
        ...
        if (! scan.nextToken.tokenStr.equals(":")) {
            throw new ParserException(
                scan.nextToken.iSourceLineNr,
                "No terminating ':'", scan.sourceFileNm,
                scan.lines[scan.nextToken.iSourceLineNr - 1]);
        }
        ...
        if (leftOpType != rightOpType) {
            if (validDataTypes.get(leftOpType) != rightOpType ||
                ...
                // throw error
                throw new ParserException(
                    rightOp.iSourceLineNr,
                    "Right datatype cannot be compared to the left",
                    scan.sourceFileNm, scan.lines[rightOp.iSourceLineNr - 1]);
            }
        }
    ...
    public int getLiteralType (String litToken) {
        ...
        else if (array.length < 2 && (array[0] == 'T' || array[0] == 'F')) {
