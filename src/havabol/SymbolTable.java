...
public class SymbolTable {
    ...
    public void putSymbol(Scanner scan) throws Exception {
        Token typeToken = scan.currentToken, token = scan.nextToken;
        String symbol = token.tokenStr;
        int prim = token.primClassif, sub = token.subClassif, type = Token.VOID;
        if (typeToken.tokenStr.equals("Int"))
            type = Token.INTEGER;
        else if (typeToken.tokenStr.equals("Float"))
            type = Token.FLOAT;
        else if (typeToken.tokenStr.equals("Bool"))
            type = Token.BOOLEAN;
        else if (typeToken.tokenStr.equals("String"))
            type = Token.STRING;
        else if (typeToken.tokenStr.equals("Date"))
            type = Token.DATE;

        if (prim == Token.OPERAND && sub == Token.IDENTIFIER) {
            table.put(symbol, new STIdentifier(symbol, prim, sub));
        } else if (prim == Token.OPERAND) {
            STEntry entry = new STEntry(symbol, prim, sub);
            if (sub == Token.INTEGER)
                entry.subClassifStr = "INTEGER";
            else if (sub == Token.FLOAT)
                entry.subClassifStr = "FLOAT";
            else if (sub == Token.STRING)
